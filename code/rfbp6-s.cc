...
elem el;
vector<elem> found;
long int ite,ite1,ite2,c,rmax,k,ltmax; 
string chunkl,chunkr; 
...
bool bm1p(string ss,node& auto0)
 {long int i,j,n=0,ii=0,jj,jl,jl1,jjj,k,kk,nk;int repmax,il,iil,ir,iir,irep; vector <short int> bm1l;bm1l.push_back(0);
  while(ii<ss.length())
      {if(ss[ii]!='{'){n=(w[ss[ii]]+n)%(b-1);ii++;continue;} if((ss[ii]=='{')&&(ss[ii+1]=='}')){ii=ii+2;continue;}
       jjj=bm1l.size();for(jl=0;jl<jjj;jl++)bm1l[jl]=((bm1l[jl]+n)%(b-1));n=0; jj=ii+1;while(ss[jj]!='}')jj++;
       for(i=ii+1;i<jj;i++)if(ss[i]!='0')
         {k=w[ss[i]];repmax=b; for (ite=0;ite!=auto0.select.size();ite++)
           {il=0;iil=0;ir=ss.length()-1;iir=found[auto0.select[ite]].t.size()-1;
        while(il<ii){if(ss[il]=='{'){il++;while(ss[il]!='}')il++;il++;continue;}if(ss[il]==tr[found[auto0.select[ite]].t[iil]])iil++;il++;}
        while(ir>jj){if(ss[ir]=='}'){ir--;while(ss[ir]!='{')ir--;ir--;continue;}if(ss[ir]==tr[found[auto0.select[ite]].t[iir]])iir--;ir--;}
        for(irep=iil;irep<=iir;irep++)
         if(k!=found[auto0.select[ite]].t[irep])break;
         if(irep<=iir)continue;
         if((iir-iil)<repmax)repmax=iir-iil;
           }
          if(repmax==b)
            {k=pgcd(k,b-1);if(k==1)return false;jjj=bm1l.size(); for(kk=k;kk<(b-1);kk=kk+k)
              {for(jl=0;jl<jjj;jl++)
                {nk=(kk+bm1l[jl])%(b-1); 
                 for(jl1=0;jl1<bm1l.size();jl1++) if(nk==bm1l[jl1]){jl1=-1;break;} 
                 if(jl1>=0)bm1l.push_back(nk); 
                }  
              }
            }
            else
            {for(kk=1;kk<repmax;kk++) 
              {jjj=bm1l.size(); 
               for(jl=0;jl<jjj;jl++)
                 {nk=(k*kk+bm1l[jl])%(b-1); 
                  for(jl1=0;jl1<bm1l.size();jl1++) if(nk==bm1l[jl1]){jl1=-1;break;} 
  ...
  jjj=bm1l.size();for(jl=0;jl<jjj;jl++){if(pgcd(b-1,bm1l[jl]+n)==1)return false;} return true;
 }
...
// bm1 en tenant compte de la limite de certaines r�p�titions
   if(xplor<=-4) if(bm1p(sl+fam(auto0.xtend)+sr,auto0)){nprune[15]++;if(nprune[15]<testprune[15]!=0)cerr<<"P15:"<<ss<<"\n";return true;}
...
// si on est dans une exploration interm�diaire, � une certaine profondeur il se peut qu'un digit de surapproximation devienne interdit; on peut alors le supprimer
  if(xplor<=-4)for (ite=0;ite!=auto0.select.size();ite++)
    {i=0;j=0;ss=sl+sr;
     while(j<ss.size())
        {if(i==found[auto0.select[ite]].t.size()){nprune[13]++;modif=true;if(nprune[13]<testprune[13])cerr<<"P13:"<<ss<<'\n';return true;}
         if(ss[j]!='{'){if(ss[j]==tr[found[auto0.select[ite]].t[i]])i++; j++;continue;} j++;
         while(ss[j]!='}'){ii=i;if(ss[j]==tr[found[auto0.select[ite]].t[i]])ii++;jj=j+1;
                           while(ss[jj]!='}')jj++;jj++;
                           while(jj<ss.size())
                               {if(ii==found[auto0.select[ite]].t.size())break;
                                if(ss[jj]=='{'){jj++;while(ss[jj]!='}')jj++;jj++;continue;}
                                if(ss[jj]==tr[found[auto0.select[ite]].t[ii]]) ii++;
                                jj++;
                               }
                           if(ii==found[auto0.select[ite]].t.size())
                                  {nprune[14]++;modif=true;if(nprune[14]<testprune[14])cerr<<"P14:"<<(sl+sr)<<'\\'<<j<<':'<<ss[j]<<'\n';
                                   if(j<sl.size())sl.erase(j,1);else sr.erase(j-sl.size(),1); break;}
                           j++;
                          }
         if(modif)break;
         j++;
    ...
    {sd="";xti.clear();
     for(iti0=auto0.xtend.begin();iti0!=auto0.xtend.end();++iti0)
        {for(iti2=auto0.xtend.begin();iti2!=auto0.xtend.end();++iti2)
         ...
         if(iti2==auto0.xtend.end())sd=sd+tr[*iti0];else xti.push_back(*iti0);
        }test=(sd.length()>0)&&(sd.length()<auto0.xtend.size());if(sd.length()!=0)sd='{'+sd+'}';
     if(xplor<0)
      {if(auto0.xtend.empty()) return false; else
      {if(test)
         {for(iti2=xti.begin();iti2!=xti.end();++iti2)
          ...
          auto1=auto0;auto1.sens=false;auto1.xtend.clear();
          if(printstring(auto1,sl+sd,sr,xplor-1))
           {nprune[12]++;if(nprune[12]<testprune[12])cerr<<"P12:"<<sl+sd<<"{forward}"<<sr<<"\n";return true;} 
          else return false;
         }  // else test
       for(iti2=auto0.xtend.begin();iti2!=auto0.xtend.end();++iti2)
           {auto1=auto0;auto1.sens=false;
            ...
            for(iti3=auto1.xtend.begin();iti3!=auto1.xtend.end();++iti3)
               if(excl[*iti2][*iti3]){iti3=auto1.xtend.erase(iti3);iti3--;}
            if(!printstring(auto1,sl+tr[*iti2],sr,xplor-1))return false;
           }
          auto1=auto0;auto1.sens=false;auto1.xtend.clear();
          return printstring(auto1,sl,sr,xplor-1);
      }
      }
     else // xplor=0
      {if(test)
       {for(iti2=xti.begin();iti2!=xti.end();++iti2)
        ...
        if(iti2==xti.end()){auto1=auto0;auto1.sens=false;auto1.xtend.clear();if(printstring(auto1,sl+sd,sr,-1))
           {nprune[12]++;if(nprune[12]<testprune[12])cerr<<"P12:"<<sl+sd<<"{forward}"<<sr<<"\n";return true;}} 
       }
      } // else on continue le tagging
    }else // sens oppos�
    {sd="";xti.clear();
     for(iti0=auto0.xtend.begin();iti0!=auto0.xtend.end();++iti0)
        {for(iti2=auto0.xtend.begin();iti2!=auto0.xtend.end();++iti2)
     ...
     if(xplor<0)
      ...
      {if(test)
         {for(iti2=xti.begin();iti2!=xti.end();++iti2)
          ...
          auto1=auto0;auto1.sens=true;auto1.xtend.clear();
          if(printstring(auto1,sl,sd+sr,xplor-1))
           {nprune[12]++;if(nprune[12]<testprune[12])cerr<<"P12:"<<sl<<"{backward}"<<sd+sr<<"\n";return true;} 
          else return false;
         } //else test
       for(iti2=auto0.xtend.begin();iti2!=auto0.xtend.end();++iti2)
           {auto1=auto0;auto1.sens=true;
      ...
      {if(test)
       {for(iti2=xti.begin();iti2!=xti.end();++iti2)
        ...
        if(iti2==xti.end()){auto1=auto0;auto1.sens=true;auto1.xtend.clear();if(printstring(auto1,sl,sd+sr,-1))
           {nprune[12]++;if(nprune[12]<testprune[12])cerr<<"P12:"<<sl+sd<<"{forward}"<<sr<<"\n";return true;}}
       }
      } // else on continue le tagging
     }
...
{string p,sr,sl;long int k,kkk;node auto0,auto1;
 ...
 ltmax=1;
 if(chunkl!="")cerr<<" pour les chiffres initiaux "<<chunkl;
 if(chunkr!="")cerr<<" et pour les chiffres terminaux "<<chunkr;
 cerr<<"\n";
           ...
           {if(!ok(ir))
              {if(pgcd(ir,b)==1)
                 {auto0.xtend.clear(); auto0.ptrl.clear(); auto0.ptrr.clear(); auto0.select.clear();auto0.sens=true;
                  ...
                  if(ite2==found.size()) // on n'a pas un premier des le depart 
                    {sr=tr[ir];sl=tr[il]; printstring(auto0,sl,sr,lt);}

                 }}}}}return;}
 auto0.ptrl.clear(); 
 for (ite2=0;ite2!=found.size();++ite2)auto0.ptrl.push_back(0);
 i=0;sl="";while(i<chunkl.length())
      {if(i==(chunkl.length()-1)||(chunkl[i+1]==':'))
         {il=chunkl[i]-'0';i++;}else{il=(chunkl[i]-'0')*10+chunkl[i+1]-'0';i=i+2;}
       if(chunkl[i]==':')i++;sl=sl+tr[il];
       for (ite2=0;ite2!=found.size();++ite2)
           {if(il==found[ite2].t[auto0.ptrl[ite2]])auto0.ptrl[ite2]++;}
      }
      auto0.xtend.clear();for(j=0;j<b;j++)if(!ok(j))auto0.xtend.push_back(j);
 if(chunkr=="")
   {for(ir=1;ir<b;ir++)
           {if(!ok(ir))
              {if(pgcd(ir,b)==1)
               {auto0.ptrr.clear(); auto0.select.clear();auto0.sens=true;
                  for (ite2=0;ite2!=found.size();++ite2)
                      {ii=found[ite2].t.size()-1;
                       if(ir==found[ite2].t[ii])auto0.ptrr.push_back(ii-1); 
                         else auto0.ptrr.push_back(ii);
                       if(auto0.ptrl[ite2]<=auto0.ptrr[ite2])
                          auto0.select.push_back(ite2); else break;
                      }
                if(ite2==found.size()){sr=tr[ir]; printstring(auto0,sl,sr,lt);}
               }}}return;
   }
   auto0.xtend.clear(); auto0.ptrr.clear(); auto0.select.clear();auto0.sens=true;
   j=chunkr.length()-1;sr="";while(j>=0)
      {if(j==0||(chunkr[j-1]==':')){ir=chunkr[j]-'0';j--;}
          else{ir=(chunkr[j-1]-'0')*10+chunkr[j]-'0';j=j-2;}
       sr=tr[ir]+sr;if(chunkr[j]==':')j--;
       for (ite2=0;ite2!=found.size();++ite2)
            auto0.ptrr.push_back(found[ite2].t.size()-1);
       for (ite2=0;ite2!=found.size();++ite2)
           {if(ir==found[ite2].t[auto0.ptrr[ite2]])auto0.ptrr[ite2]--;}
      }
     for(j=0;j<b;j++)if(!ok(j))auto0.xtend.push_back(j);
     for (ite2=0;ite2!=found.size();++ite2)
         if(auto0.ptrl[ite2]<=auto0.ptrr[ite2])auto0.select.push_back(ite2); else return;
 printstring(auto0,sl,sr,lt);
//construction des familles a explorer a partir des premiers et derniers chiffres
...
{string ms,pref,prefms;
 ...
 for(int i=0;i<20;i++){testprune[i]=100000;testtag[i]=100000;}
 handlebase();
 cputime = clock()-cpustarttime; cputime=cputime/1000000;
 result<<"%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n";
 result<<"\nA la fin, on a exploré "<<nn<<" familles de nombres pour la base "<<b;
 if(chunkl!="")result<<" et le chiffre initial "<<chunkl;
 if(chunkr!="")result<<" et le chiffre terminal "<<chunkr;
 result<<"\n on a pu appliquer en pruning ";
 for (int i=0;i<20;i++)if(nprune[i]!=0)result<<nprune[i]<< " règles "<<i<<" , ";
 result<<"\n on a pu appliquer en tagging ";
 for (int i=0;i<20;i++)if(ntag[i]!=0)result<<ntag[i]<< " règles "<<i<<" , ";
 result<<"\n en "<<cputime<<" secondes\n"; 
 ...
 if(nt>0){cerr<<"on a effectué "<<nt<<" troncatures "<<"à la taille "<<lt<<" pour la base "<<b;
          if(chunkl!="")cerr<<" et le chiffre initial "<<chunkl;
          if(chunkr!="")cerr<<" et le chiffre terminal "<<chunkr;cerr<<"\n";
