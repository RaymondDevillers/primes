...
elem el;
vector<elem> found;
long int ite,ite1,ite2,c,rmax,k,jj,chunkl,chunkr,deepm; 
...
void printstring(node auto0, string sl, string sr, int deep)
 {list<short int>::iterator iti,iti0,iti1,iti2,iti3; string ss,ss1,ss2,sso,sd;vector<int>loop,nl;
  ...
  if(deep>deepm)deepm=deep;
//cerr<<'\n';for (ite=0;ite!=auto0.select.size();ite++) cerr<<auto0.select[ite]<<'/'<<auto0.ptrl[ite]<<','<<auto0.ptrr[ite]<<'\n';
  for(iti=auto0.xtend.begin();iti!=auto0.xtend.end();iti++)
     {i=*iti;   // on allonge virtuellement pour detecter les extensions permises
...
//  cas des extensions uniques
  if(auto0.xtend.size()==1)
    ...
    // sinon
     ...
     if(rmax==20000){printres(ss);return;}
...
//  famille encore à considérer       
  ...
  if(nl.empty())return;
// rejet car trop long
  if(auto0.xtend.size()>1)if(deep==lt){nt++;ss=sl+'<';
    ...
    {if(!loop.empty()){sl=sl+'{';for(i=0;i<loop.size();i++)sl=sl+tr[loop[i]];sl=sl+'}';}
     for(i=0;i<nl.size();i++)
        {auto1.sens=false;auto1.xtend=auto0.xtend;auto1.select.clear();auto1.ptrl.clear();auto1.ptrr.clear();
         ...
         printstring(auto1,sl+tr[nl[i]],sr,deep+1);
        }
    }else
              ...
              {if(pgcd(ir,b)==1)
                 {auto0.xtend.clear(); auto0.ptrl.clear(); auto0.ptrr.clear(); auto0.select.clear();auto0.sens=true;
                  ...
                  if(ite2==found.size()) // on n'a pas un premier à 2 chiffres
                    {sr=tr[ir];sl=tr[il]; printstring(auto0,sl,sr,0);}
//construction des familles à explorer à  partir du premier et du dernier chiffre
 ...
 result<<"\n en "<<cputime<<" secondes\n"; 
 cerr<<"\nla taille maximum considérée a été de "<<deepm<<"\n";
 result<<"la taille maximum considérée a été de "<<deepm<<"\n";
 result.flush();
