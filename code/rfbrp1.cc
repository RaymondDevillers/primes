...
bool bp1(string s)
 {int i,j,n=0,nn=1,ii=0,jj,jl,jl1,jjj,k,kk,nk;vector <short int> bp1l;bp1l.push_back(0);return false;
//cerr<<"\n"<<s<<':';
  while(ii<s.length())
...
// trace
  ...
  if(xplor>=0)nn++;
  if((nn%1)==0){cerr<<nn;if(xplor<0)cerr<<'.'<<-xplor;cerr<<" ("<<b<<"):"<<sso<<"\n";cerr.flush();}  //if((nn%10000)==0)
// pruning rules
           ...
           {if(!ok(ir))
              {if(pgcd(ir,b)==1)
                 {auto0.xtend.clear(); auto0.ptrl.clear(); auto0.ptrr.clear(); 
                  for(j=0;j<b;j++)if(!ok(j))auto0.xtend.push_front(j);
...
{string ms,pref,prefms;
 ...
 pref="resfbR"; prefms=pref+ms; 
 result.open(prefms.c_str(),ios::out);
 pref="xfbR"; prefms=pref+ms;
 court.open(prefms.c_str(),ios::out);
 pref="rejfbR"; prefms=pref+ms;
