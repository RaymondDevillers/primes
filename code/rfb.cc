...
elem el;
vector<elem> found;
int ite,ite1,ite2,c,rmax,k,chunkl,chunkr,ltmax; 
...
bool bp1(string s)
 {int i,j,n=0,nn=1,ii=0,jj,jl,jl1,jjj,k,kk,nk;vector <short int> bp1l;bp1l.push_back(0);
//cerr<<"\n"<<s<<':';
  while(ii<s.length())
...
bool bm1(string s)
 {int i,j,n=0,ii=0,jj,jl,jl1,jjj,k,kk,nk;vector <short int> bm1l;bm1l.push_back(0);
  while(ii<s.length())
...
//cerr<<"!"<<xti.size()<<'/'<<rest.size()<<':';for(iti2=xti.begin();iti2!=xti.end();++iti2)cerr<<tr[*iti2];cerr<<";";for(iti2=rest.begin();iti2!=rest.end();++iti2)cerr<<tr[*iti2];cerr<<"\n";
   while(true) 
...
printstring(node auto0, string sl, string sr)
 {list<short int>::iterator iti,iti0,iti1,iti2,iti3; string ss,ss1,ss2,sso,sd;vector<int>loop,nl;
  list <short int> xti; bool testl,final; int i,j,jl,ii,i1,i2; 
  ...
  for(iti=auto0.xtend.begin();iti!=auto0.xtend.end();iti++)
     {i=*iti;   // on allonge virtuellemnt pour detecter les extensions permises
      for (ite=0;ite!=found.size();ite++)
          {if((auto0.ptrl[ite]==auto0.ptrr[ite])&&(i==found[ite].t[auto0.ptrl[ite]]))break;}
      if(ite!=found.size()){iti=auto0.xtend.erase(iti);iti--;} // couvert
...
// relations entre les extensions permises
  ...
  for (ite=0;ite<found.size();ite++)
    if(auto0.ptrr[ite]==(1+auto0.ptrl[ite]))
      {excl[found[ite].t[auto0.ptrl[ite]]][found[ite].t[auto0.ptrr[ite]]]=true;
       for(k=0;k<basemax;k++)
          {excl2[found[ite].t[auto0.ptrl[ite]]][found[ite].t[auto0.ptrr[ite]]][k]=true;        
           excl2[found[ite].t[auto0.ptrl[ite]]][k][found[ite].t[auto0.ptrr[ite]]]=true;
           excl2[k][found[ite].t[auto0.ptrl[ite]]][found[ite].t[auto0.ptrr[ite]]]=true;
          }
      }
    else if(auto0.ptrr[ite]==(2+auto0.ptrl[ite]))   
           excl2[found[ite].t[auto0.ptrl[ite]]][found[ite].t[1+auto0.ptrl[ite]]][found[ite].t[auto0.ptrr[ite]]]=true;
// trace
...
// si un des digits d ne peut apparaître plus de k fois (k>1, sinon on est dans un cas précédent, et pas trop grand sinon la règle des petits facteurs risque de ne pas donner grand chose) et si, pour chaque répétition possible, il y a des petits facteurs, on peut supprimer d
  for(iti=auto0.xtend.begin();(auto0.xtend.size()>1)&&(iti!=auto0.xtend.end());iti++)if(!excl[*iti][*iti])
     {rmax=100;i=*iti;
      for(ite=0;ite<found.size();ite++)
         {for(j=auto0.ptrl[ite];j<=auto0.ptrr[ite];j++)if(i!=found[ite].t[j])break;
     ...
     if(test)
       {for(iti2=xti.begin();iti2!=xti.end();++iti2)
           {auto1=auto0;auto1.sens=false;
            for(ite=0;ite<found.size();ite++)if((*iti2)==found[ite].t[auto1.ptrl[ite]])auto1.ptrl[ite]++;
            for(iti3=auto1.xtend.begin();iti3!=auto1.xtend.end();++iti3)
               if(excl[*iti2][*iti3]){iti3=auto1.xtend.erase(iti3);iti3--;}
               printstring(auto1,sl+sd+tr[*iti2],sr,-1);
           }
     ...
     if(test)
       {for(iti2=xti.begin();iti2!=xti.end();++iti2)
           {auto1=auto0;auto1.sens=true;
            for(ite=0;ite<found.size();ite++)if((*iti2)==found[ite].t[auto1.ptrr[ite]])auto1.ptrr[ite]--;
            for(iti3=auto1.xtend.begin();iti3!=auto1.xtend.end();++iti3)
               if(excl[*iti3][*iti2]){iti3=auto1.xtend.erase(iti3);iti3--;}
               printstring(auto1,sl,tr[*iti2]+sd+sr,-1);
           }
        if((iti2==xti.end())&&((petit_facteur(sl+sd+sr)>0)||bp1(sl+sd+sr)||bm1(sl+sd+sr)))
...
//  cas des extensions uniques
  ...
  if(auto0.sens)for(iti=auto0.xtend.begin();iti!=auto0.xtend.end();iti++)
                    {i=*iti;for (ite=0;ite<found.size();ite++)
                     if(i==found[ite].t[auto0.ptrl[ite]])break;
                     if(ite==found.size())loop.push_back(i);else nl.push_back(i);
                    }
          
           else for(iti=auto0.xtend.begin();iti!=auto0.xtend.end();iti++)
                   {i=*iti;for (ite=0;ite<found.size();ite++)
                    if(i==found[ite].t[auto0.ptrr[ite]])break;
                    if(ite==found.size())loop.push_back(i);else nl.push_back(i);
                   }
...
// rejet car trop long
  if(auto0.xtend.size()>1){nt++;ss=sl+'<';
...
// itération suivante
  if(auto0.sens)
    {if(!loop.empty()){sl=sl+'{';for(i=0;i<loop.size();i++)sl=sl+tr[loop[i]];sl=sl+'}';}
     for(i=0;i<nl.size();i++)
        {auto1=auto0;auto1.sens=false;
         for(ite=0;ite<found.size();ite++)if((nl[i])==found[ite].t[auto1.ptrl[ite]])auto1.ptrl[ite]++;
         printstring(auto1,sl+tr[nl[i]],sr);
        }
    }else
    {if(!loop.empty()){sr='}'+sr;for(i=0;i<loop.size();i++)sr=tr[loop[i]]+sr;sr='{'+sr;}
     for(i=0;i<nl.size();i++)
        {auto1=auto0;auto1.sens=true;
         for(ite=0;ite<found.size();ite++)if((nl[i])==found[ite].t[auto1.ptrr[ite]])auto1.ptrr[ite]--;
         printstring(auto1,sl,tr[nl[i]]+sr);
        }
    }
 return true;
 ...
 for(il=ilb;il<ilh;il++)
    {if(!ok(il))
       {for(ir=irb;ir<irh;ir++)
           {if(!ok(ir))
              {if(pgcd(ir,b)==1)
                 {auto0.xtend.clear(); auto0.ptrl.clear(); auto0.ptrr.clear(); 
                  ...
                  for (ite2=0;ite2!=found.size();++ite2)
                      {ii=found[ite2].t.size()-1;if(ir==found[ite2].t[ii])ii--;
                       ...
                       auto0.ptrr.push_back(ii);auto0.ptrl.push_back(kk);
                      }
